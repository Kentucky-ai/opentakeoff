...
import { TOOL_STAGES, SETUP_STAGE_TOOLS, stagedToolsEnabled, type ToolStageKey } from "./toolStages.ts";
import {
...
// #230 — every registerTool call below routes through here so registerTools
// can find each RegisteredTool again afterward (disable() on the staged
// groups, or hand back none at all when staging is off).
const _reg = (
  server: McpServer,
  registered: Map<string, RegisteredTool>,
  name: string,
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- registerTool's overloads don't
  // survive Parameters<> extraction; each call site below is still checked against the SDK's own
  // overload set the same as before this wrapper existed, this just also stashes the result.
  config: any,
  cb: any,
): RegisteredTool => {
...
export function registerTools(server: McpServer, session: Session): void {
  ...
  _reg(server, registered, "load_plan", {
  ...
  _reg(server, registered, "sheet_info", {
  ...
  _reg(server, registered, "set_scale", {
  ...
  _reg(server, registered, "one_click", {
  ...
  _reg(server, registered, "detect_rooms", {
  ...
  _reg(server, registered, "measure_polygon", {
  ...
  _reg(server, registered, "cut_out", {
  ...
  _reg(server, registered, "measure_line", {
  ...
  _reg(server, registered, "measure_surface", {
  ...
  _reg(server, registered, "place_count", {
  ...
  _reg(server, registered, "symbol_sweep", {
  ...
  _reg(server, registered, "sweep_schedule_row", {
  ...
  _reg(server, registered, "derive_base", {
  ...
  _reg(server, registered, "derive_transitions", {
  ...
  _reg(server, registered, "takeoff_summary", {
  ...
  _reg(server, registered, "export_takeoff", {
  ...
  _reg(server, registered, "export_report", {
  ...
  _reg(server, registered, "import_takeoff", {
  ...
  _reg(server, registered, "apply_rules", {
  ...
  _reg(server, registered, "export_marked_pdf", {
  ...
  _reg(server, registered, "list_shapes", {
  ...
  _reg(server, registered, "delete_shape", {
  ...
  _reg(server, registered, "sheet_context", {
  ...
  _reg(server, registered, "edit_shape", {
  ...
  _reg(server, registered, "edit_materials", {
  ...
  _reg(server, registered, "edit_condition", {
  ...
  _reg(server, registered, "duplicate_condition", {
  ...
  _reg(server, registered, "split_condition", {
  ...
  _reg(server, registered, "undo_last", {
  ...
  _reg(server, registered, "sheet_graph", {
  ...
  _reg(server, registered, "resolve_tag", {
  ...
  _reg(server, registered, "find_schedule", {
  ...
  _reg(server, registered, "read_sheet_text", {
  ...
  _reg(server, registered, "find_text", {
  ...
  _reg(server, registered, "view_sheet", {
  ...
  _reg(server, registered, "annotate", {
  ...
  _reg(server, registered, "list_annotations", {
  ...
  _reg(server, registered, "link_annotation", {
  ...
  _reg(server, registered, "mark_verdict", {
  ...
  _reg(server, registered, "delete_verdict", {
  ...
  // #230 — opt-in staged exposure. Unset (the default): every currently-
  // published client's contract is untouched, all forty tools enabled on
  // connect, exactly as before this env var existed. Set to "1": only setup
  // registers enabled; the other three stages sit disabled — present in
  // tools/list's shape but filtered out of the actual reply by the SDK — until
  // open_tool_stage flips one on and fires the list-changed notification a
  // spec-current client (Claude Code, Claude Desktop, and anything else built
  // against dynamic tool lists) re-fetches tools/list against.
  if (stagedToolsEnabled()) {
    for (const stage of TOOL_STAGES) {
      for (const name of stage.tools) registered.get(name)?.disable();
    }

    const stageByKey = new Map(TOOL_STAGES.map((s) => [s.key, s] as const));
    const opened = new Set<ToolStageKey>();

    _reg(server, registered, "open_tool_stage", {
      description: `Enable one of this session's held-back tool groups (#230 — OPENTAKEOFF_MCP_STAGED_TOOLS is set, so most of the forty tools start disabled to keep an idle session's tool list short). ${SETUP_STAGE_TOOLS.join(", ")} are ALWAYS on. Call this with the stage whose tools your next step needs: ${TOOL_STAGES.map((s) => `"${s.key}" (${s.title.toLowerCase()} — ${s.summary})`).join("; ")}. Enabling a stage is additive and sticky for the rest of this connection — re-opening an already-open stage just confirms its tool list. Every enable fires a tools/list-changed notification; a client that doesn't refetch on that notification won't see the newly-enabled tools until it calls tools/list on its own.`,
      inputSchema: {
        stage: z.enum(TOOL_STAGES.map((s) => s.key) as [ToolStageKey, ...ToolStageKey[]]).describe("Which held-back group to enable"),
      },
      outputSchema: { stage: z.string(), opened_tools: z.array(z.string()), already_open: z.boolean(), still_closed: z.array(z.string()) },
    }, run("open_tool_stage", ({ stage }: { stage: ToolStageKey }) => {
      const def = stageByKey.get(stage);
      if (!def) throw new UserError(`Unknown stage "${stage}". Valid: ${TOOL_STAGES.map((s) => s.key).join(", ")}.`);
      const alreadyOpen = opened.has(stage);
      if (!alreadyOpen) {
        for (const name of def.tools) registered.get(name)?.enable();
        opened.add(stage);
      }
      const stillClosed = TOOL_STAGES.filter((s) => !opened.has(s.key)).map((s) => s.key);
      return { stage, opened_tools: [...def.tools], already_open: alreadyOpen, still_closed: stillClosed };
    }));
  }
}
