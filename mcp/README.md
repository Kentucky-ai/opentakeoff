...
| `propose_takeoff` | Start a named batch (`label` + `rationale`) that subsequent agent commits attach to. The estimator can accept the batch as one decision. |
| `revise_proposal` | Replace a proposal's still-pending shapes with a new set in one journal step; accepted shapes are never touched. |
| `withdraw_proposal` | Withdraw every still-pending shape in a proposal in one journal step. |
| `propose_condition_edit` | Hold a condition diff for estimator acceptance. The current values remain live, so `export_report` does not change while it is pending. |
| `withdraw_condition_edit` | Withdraw a pending condition diff without changing the condition. |
