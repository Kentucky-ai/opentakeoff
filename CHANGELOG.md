...
- **Revert scale.** After any rescale that changed committed quantities (the mistyped-recalibrate case), the Scale menu surfaces a one-step **Revert scale (was …)** that puts the sheet back on the previous scale and re-prices its takeoffs. One slot, this session only (never saved) — and reverting is itself revertible.
