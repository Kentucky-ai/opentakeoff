...
const reference = JSON.parse(readFileSync(resolve(here, "reference.json"), "utf8"));
...
async function workflow(staged, dir) {
  ...
  try {
    ...
    const sheet = loaded.value?.sheets?.[0]?.sheet;
    if (sheet !== "sample-plan.pdf") throw new Error(`unexpected loaded sheet identity: ${sheet}`);
    ...
    await tool("propose_takeoff", { label: "Synthetic four-room wall-face areas", rationale: "Analytic inset wall faces from demo/sample-plan.pdf; scripted conformance fixture." });
    for (const room of reference.rooms) await tool("measure_polygon", { sheet, verts: room.verts_px, condition: "SYNTH-FLOOR", role: "floor_area" });
    if (staged) {
    ...
    if (shapeRows.length !== 4) throw new Error(`expected four committed shapes, got ${shapeRows.length}`);
    ...
    if (exportPayload.shapes?.length !== 4) throw new Error("editable export does not contain four shapes");
...
const summary = { benchmark: "scripted MCP workflow conformance", node_version: process.version, server_path: serverPath, reference_id: reference.reference_id, source_sha256: planHash, analytic_reference: true, independently_human_reviewed: false, boundary: "synthetic analytic fixture; no claim of real-plan accuracy or human review", cross_mode_canonical_equal: crossModeEqual, runs, fresh_process: fresh };
